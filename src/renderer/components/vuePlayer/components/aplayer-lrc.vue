<template>
  <div class="lyrics-wrapper aplayer-lrc">
    <ul class="content aplayer-lrc-contents" :style="transformStyle">
      <li
...
//     "mousemove",
//     function(e) {
//       if (isDown&&ctx.contains(e.target)) {
//         //如果鼠标是从容器里开始滑动的
//         if (e.timeStamp - this._startTime > 40) {
//           //如果是慢速滑动，就不会产生力度，列表是跟着鼠标移动的
...
export default {
  ...
  mounted() {
    // myScroll(document.querySelector(".lyrics-wrapper"));
  },
  methods: {
    applyLrc(lrc) {
      if (/^https?:\/\//.test(lrc)) {
